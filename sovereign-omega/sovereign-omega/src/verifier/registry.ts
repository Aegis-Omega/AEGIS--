...
class CapacityDeclarationRegistry {
  ...
  async register(declaration: CapacityDeclaration): Promise<void> {
    ...
    const depHash = await hashValue({ operators: declaration.mutation_operators }) as SHA256Hex
    if (depHash !== declaration.dependency_graph_hash)
      throw new RegistrationError(`Dependency graph hash mismatch for ${declaration.component_id}`)
